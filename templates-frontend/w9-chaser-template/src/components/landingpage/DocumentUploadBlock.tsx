...
export const DocumentUploadBlock: React.FC<DocumentUploadBlockProps> = ({
  block,
  notaryPageId,
}) => {
  const { value } = block;
        ...
        {success ? (
          <div className="text-center">
            ...
            <p className="text-xl text-gray-600 mb-8">
              Your documents have been securely uploaded.
            </p>
            <div className="flex gap-4 justify-center">
              <button
                onClick={handleUploadMore}
                className="px-8 py-4 bg-gradient-to-r from-theme-primary to-theme-secondary text-white rounded-xl font-bold text-lg hover:shadow-xl transition-all"
              >
                Upload More Documents
              </button>
              <button
                onClick={handleCancel}
                className="px-8 py-4 bg-gray-200 text-gray-900 rounded-xl font-bold text-lg hover:bg-gray-300 transition-all"
              >
                Return to Homepage
              </button>
            </div>
          ...
          <>
              ...
              <div className="pointer-events-none">
                ...
                <p className="text-gray-600 mb-4 text-lg">
                  ...
                  <span className="font-bold text-theme-primary">
                    {value.allowed_file_types.join(", ")}
                  </span>
                </p>

                <p className="text-sm text-gray-500">
                  Maximum file size:{" "}
                  <span className="font-bold">{value.max_file_size}MB</span>
                </p>
              </div>
            </div>
            ...
            {files.length > 0 && (
              <div className="mt-8 space-y-4">
                {files.map((file, index) => (
                  <div
                    key={index}
                    className="flex items-center gap-4 p-6 bg-white rounded-2xl shadow-lg border-2 border-green-100 hover:shadow-xl transition-all"
                  >
            ...
            <button
              ...
              disabled={
                uploading || files.length === 0 || !clientName || !clientEmail
              }
            ...
            >
              {uploading ? "Uploading..." : "Upload Documents Securely"}
            </button>
          </>
        )}
      </div>
