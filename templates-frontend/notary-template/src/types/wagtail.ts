...
export interface ServiceItem {
  ...
  cta_action: string; // 'url' | 'book' | 'contact' | ''
  cta_target?: string; // URL target if cta_action is 'url'
  is_popular: boolean;
}
...
export const fetchNotaryPageData = async (): Promise<NotaryPageData> => {
  ...
  try {
    ...
    const apiBlocks: NotaryBlock[] = [
      ...
      page.service_area &&
        page.service_area.cities &&
        page.service_area.cities.length > 0 && {
      ...
      ...dynamicBlocks,
      page.booking &&
        page.booking.duration_options &&
        page.booking.duration_options.length > 0 && {
      ...
      page.upload &&
        page.upload.allowed_types &&
        page.upload.allowed_types.length > 0 && {
      ...
      page.testimonials &&
        ((page.testimonials.items && page.testimonials.items.length > 0) ||
          page.testimonials.display_type === "google") && {
      ...
      page.payment &&
        page.payment.amount &&
        parseFloat(page.payment.amount) > 0 && {
          type: "payment" as const,
          value: {
            payment_type: page.payment.type || "full",
            amount: page.payment.amount || "0",
            description: page.payment.description || "",
            require_before_proceeding:
              page.payment.require_before_proceeding || false,
            success_message: page.payment.success_message || "",
          },
          id: "payment-1",
        },
      page.contact_form &&
        page.contact_form.fields &&
        page.contact_form.fields.length > 0 && {
    ...
    const mockBlocksToKeep = mockNotaryData.blocks.filter(
